...
void main()
{
...
#ifdef FULLSCREEN
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "Viridian Serpent", glfwGetPrimaryMonitor(), 0 );
#else
#ifdef DOUBLESIZE
	window = glfwCreateWindow( SCRWIDTH * 2, SCRHEIGHT * 2, "Viridian Serpent", 0, 0 );
#else
	window = glfwCreateWindow( SCRWIDTH, SCRHEIGHT, "Viridian Serpent", 0, 0 );
#endif
#endif
	...
	char dir[2048];
	printf( "Running Viridian Serpent on directory: %s\n", getcwd( dir, 2048 ) );
#endif
	...
	// Set window so it can be accessed by m_ui_manager to quit the game
	app->m_window = window;
	...
	// close down
	app->Shutdown();
	delete renderTarget; //  Fixes memory leak for VLD
	delete shader; //  Fixes memory leak for VLD
	delete screen; //  Fixes memory leak for VLD
	delete app;
	Kernel::KillCL();
